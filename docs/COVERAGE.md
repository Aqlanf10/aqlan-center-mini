...
> 🔴 لم يبدأ · ⚪ لا ينطبق بعد. آخر فحص: بعد دمج تحديثات المالك (اكتمال المرحلة ٢ + المرحلة ٧ + بنية المستندات) — typecheck ✅ · 250/250 اختبار ✅ · build ✅.
...
| | التحليل السيفالومتري | — (النموذج المعتمد WebCeph) | 🔴 |
...
| 6 | Imaging | `patient_documents` (وصف فقط) · `lib/files.ts`/`storage.ts`/`tar.ts` · `app/api/documents/` — التحليل السيفالومتري لاحقاً | 🟡 |
...
| 8 · السيفالو والأشعة | 🟡 | بنية المستندات والتخزين الخارجي جاهزة ومفحوصة؛ يبقى التحليل السيفالومتري (WebCeph) |
| 9 · المخزون | 🔴 | |
...
| 10 | منطق أعمال في React | المنطق في `lib/` مُختبَراً (250 اختباراً)؛ بقايا تجميع عرضي في أربع شاشات قيد التنظيف | 🟡 |
...
| ترتيب المراحل | 2 قبل 3-6 | 3-6 قبل إكمال 2، ثم اكتملت ٢ و٧ | بُنيت قبل اعتماد الدستور؛ الهدم أضرّ من الانحراف — والترتيب من هنا ملتزم |
