...
### TD-02 — EXECUTED (owner-authorized phase, branch `td/02-environment-ci-parity` from main `831851f`)

Executed per the owner's TD-02 instruction (AUDIT → REUSE → EXTEND → TEST → DOCUMENT), closing TD-REG-019 and TD-REG-013, closing TD-REG-008's test/development parity scope, and partially closing TD-REG-010 (see below). Outcome vs plan: (a) local PG18 path shipped as `docker-compose.yml` (`pg18` = postgres:18-alpine with the CI env shape, host port 54329); (b) `schema:contract` fails closed on a non-18 server before writing `*.pg18.json` (stronger than the planned "warn"); (c) `.nvmrc` + engines (node `>=22 <23`, npm `>=10.9 <12`) + `lib/env-contract.ts` as the single contract source; (d) the full-tier command is `npm run verify:full` — one canonical gate running **every essential fail-able CI gate** in CI's own order (13 steps: environment → typecheck → lint → unit → postgres → schema-contract generation → journeys → baseline-manifest generation → **committed-baseline verification** → audit → body-scan → build → HTTP-security), with a fail-closed environment preflight (`npm run verify:environment`) that requires the documented two-URL setup (`TEST_DATABASE_URL` = integration test database, `DATABASE_URL` = maintenance database for operational journeys) and inspects every runtime connection alias; the only CI-only remainder is artifact upload and Chromium install (provenance/environment prep — see the CI-vs-local matrix in `docs/ENVIRONMENT_CI_PARITY.md`); (e) staging: `docs/STAGING_READINESS.md` defines the contract and proves the ephemeral controlled-environment rehearsal path — the persistent Railway staging resource is NOT provisioned (STAGING INFRASTRUCTURE OWNER APPROVAL REQUIRED, a decision reserved to the owner). CI gained one early step (`verify:environment`) and lost none; all mandatory gates remain individually visible. New `__tests__/environment-parity.test.ts` (39 tests) makes the contracts executable: Node/npm/PG18/timezone/DB-URL-safety invariants, CI-gate non-disappearance, Docker≡CI≡engines Node agreement, and `.env.example` secret hygiene. `docs/ENVIRONMENT_CI_PARITY.md` is the environment ownership matrix (before/after). Zero migrations, no production DB access, no Railway production changes, TD-01A/TD-01B/TD-08A not started.

**Owner review corrections (same PR #45, TD-02 scope discipline kept):** (1) the documented fresh-setup path was rewired — the preflight of `verify:full` now requires BOTH URLs explicitly (`TEST_DATABASE_URL` = integration test database, `DATABASE_URL` = maintenance database for operational journeys; journeys read `DATABASE_URL` only, so the old TEST-only documented command could pass preflight then SKIP journeys), `USE_LOCAL_DB=true` is rejected up front, and the documented one-command setup was proven end-to-end; (2) the fail-closed DB-safety check now covers every runtime connection alias (`DATABASE_URL`, `POSTGRES_URL`, `POSTGRES_PRISMA_URL`, `POSTGRES_URL_NON_POOLING` — one shared exported authoritative list `RUNTIME_DATABASE_URL_ENV_NAMES` consumed by both `lib/db.ts` and the gate — plus `TEST_DATABASE_URL` and `SOURCE_DATABASE_URL`), closing the gap where a remote/Railway URL through an uninspected alias passed preflight; (3) `verify:full` gained the CI schema/baseline safety gates (schema-contract generation with committed-file restore, baseline-manifest generation, **committed-baseline verification**) and the claim was made exact: every essential fail-able CI gate local, artifact uploads documented as CI-only provenance; (4) unverified production-PostgreSQL claims were removed from `ci.yml` comments and TD-02 docs (supported CI/test/staging contract = PostgreSQL 18; the production Railway major is NOT claimed without fresh evidence); (5) TD-REG-025 reclassified P3 → P2 (possible payment-amount integrity impact — production impact not proven, focused reproduction/instrumentation required before go-live; TD-05 product code untouched); (6) the compose teardown command was corrected (`docker compose down -v` — no service argument; `down -v pg18` was invalid syntax).
