...
## Finance module — التشغيل المالي (2026-08)

### دورة الإقفال اليومي (نهاية كل يوم عمل)
1. مدير النظام يفتح **المالية ← التقارير المالية ← الإقفال اليومي**.
2. يراجع لكل صندوق: الرصيد الافتتاحي، صافي الحركة، الرصيد الختامي — لكل عملة على حدة (لا تجميع بين العملات أبدًا).
3. يتحقق من تفصيل طرق الدفع (نقد/تحويل/بطاقة) وأن بند «تحصيلات مرضى بدون سند (نظام قديم)» يطابق الدفعات القديمة إن وجدت.
4. يطبع التقرير (A4) للأرشيف الورقي: `/print/daily-closing?date=YYYY-MM-DD`.
5. أي فرق: يفتح سجل سندات القبض وسجل سندات الصرف ويعاين الحركات المرتبطة بالتاريخ نفسه.

### تصحيح سند خاطئ (عكس السند)
- لا يُعدَّل ولا يُحذف سند بعد إنشائه أبدًا.
- ADMIN يفتح السند من السجل ← «عكس السند» ← سبب إلزامي ← يتم إنشاء حركة مقابلة مرتبطة بالأصل وتعليم الأصل REVERSED.
- عكس سند قبض مريض يعيد المبلغ لرصيد المريض تلقائيًا (حركة دفع سالبة مقابل نفس السند).
- عكس سند مرتبط بعمولة تحصيل يعكس العمولة تلقائيًا في نفس المعاملة.
- عكس الحركة العكسية نفسها ممنوع (يفحصه الخادم).

### دورة العمولات
1. إكمال زيارة فيها بنود عمل «مؤهلة للعمولة» ينشئ عمولات PENDING داخل نفس معاملة الإكمال (لقطة الخطة تحفظ لحظة الإنشاء).
2. بدون خطة مضبوطة تبقى العمولة PENDING بدون مبلغ — لا استحقاق ولا دفع حتى يحدد ADMIN المبلغ ويعتمده.
3. ADMIN: المالية ← العمولات ← «تحديد المبلغ» ← «اعتماد» ← «دفع بسند صرف» (ينشئ سند صرف مرتبطًا ويحوّل الحالة PAID).
4. الطبيب يرى أعماله وعمولاته فقط من «أعمالي وعمولاتي» — بدون أي وصول لبيانات غيره.

### دورة المعامل
1. إنشاء حالة معمل (مريض/طبيب/خدمة/تكلفة) ← تتبع الحالة (مطلوبة/مرسلة/مستلمة/مسلمة).
2. عند وصول فاتورة المعمل: «تسجيل فاتورة» على الحالة — بعدها تدخل التكلفة في رصيد المعمل (لا يمكن إعادة التسجيل).
3. الدفع للمعمل بسند صرف (ADMIN) — الرصيد المتبقي يظهر في صفحة المعامل وفي التقرير المالي للفترة.
4. كشف حساب المعمل قابل للطباعة (A4).

### دورة الموردين
1. إضافة موردين ومواد ← فاتورة شراء متعددة البنود (الإجمالي يحسبه الخادم).
2. الدفع للمورد بسند صرف مرتبط بالفاتورة أو بحساب المورد العام.
3. إلغاء فاتورة (بسبب إلزامي) ممكن فقط ما دامت بلا مدفوعات صافية.
4. كشف حساب المورد قابل للطباعة (A4).

### ملاحظات أمان مالية
- كل حركة مالية + سجل التدقيق في معاملة واحدة (transaction) — لا يوجد سند بدون أثر.
- مفاتيح Idempotency تمنع تكرار السند عند النقر المزدوج أو إعادة الإرسال.
- RECEPTION: إنشاء سندات قبض المرضى وطباعتها فقط. DOCTOR: لا ينشئ سندات مالية إطلاقًا.
- إعادة الطباعة تُسجل في سجل التدقيق (PRINTED/REPRINTED) بدون بيانات حساسة.
- أرشفة الخدمات/التصنيفات/الخزائن/المعامل/الموردين بدل الحذف دائمًا.
