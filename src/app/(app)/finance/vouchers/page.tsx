...
export default async function PaymentVouchersPage({
  ...
  const filtered = q
    ? rows.filter(
        (row) =>
          ...
          (row.otherPartyName ?? "").toLowerCase().includes(q) ||
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={dict.financeVouchers.vouchersTitle}
        subtitle={dict.financeVouchers.vouchersSubtitle}
        actions={
          <PaymentVoucherDialog
            ...
            <tbody>
              {filtered.map((row) => (
                <tr key={row.id} className="border-b last:border-0">
                  ...
                  <td className="px-3 py-2.5">
                    {dict.financeVouchers.paymentMethods[row.paymentMethod as "CASH"]}
                  </td>
                  <td className="px-3 py-2.5">
                    {row.status === "REVERSED" ? (
                  ...
                  <td className="px-3 py-2.5">
                    <div className="flex flex-wrap items-center gap-1.5">
                      ...
                      {row.status === "ACTIVE" && !row.reversalOfVoucherId ? (
                        <VoucherReversalDialog
                          voucherId={row.id}
