 ...
 *  7.  تحقق انحراف عقد المخطط الحالي (schema:contract:verify)
 *      — **بوابة أمان** لا خطوة رفع أثر: مطابقة النسخة الطازجة المحفوظة
 *      في مجلد الأثر بالملف الملتزم **بنيويًّا حرفيًّا** (canonical) — أي
 *      فرق جدول/عمود/نوع/قيد/فهرس/مُشغّل/عدّاد يُفشل البوابة بـ
 *      SCHEMA_CONTRACT_DRIFT. مصدر الإصدار (minor للخادم: 18.4 مقابل
 *      18.6 مثلًا) يُقال ولا يُقارن — عين ترتيب CI حيث المقارنة تجري قبل
 *      استعادة الملتزم فوق شجرة العمل.
 *  8.  رحلات التحقق التشغيلية (verify:ci) — وتشمل رحلة verify:schema
 *      التي تطابق البناء الطازج بالعقد الملتزم (مجموعة جزئية — حماية
 *      مستقلة باقية كما هي)                           — كما في CI
 *  9.  توليد بيان خط الأساس المرشّح (db:baseline:manifest)    — كما في CI
 *      إلى ملف مؤقت خارج المستودع (لا يلمس الملتزم).
 * 10.  تحقق بيان خط الأساس الملتزم ضد التوليد الطازج
 *      (db:baseline:manifest:verify --fresh) — **بوابة أمان**، لا خطوة رفع أثر.
 * 11.  تدقيق الاعتماديات (ci:audit)                           — كما في CI
 * 12.  ماسح قارئات الأجسام (ci:scan:body)                     — كما في CI
 * 13.  بناء الإنتاج (build)                                    — كما في CI
 * 14.  اختبارات الأمن على HTTP (test:security-http)            — كما في CI
 *
