-- 0005 — Financial Append-Only Integrity (Trigger Guards)
-- المبدأ (P1.6): الحدث المالي التاريخي غير قابل للتعديل. التصحيح حدثٌ صريح
-- لاحق (ردّ/تسوية)، لا UPDATE صامت يغيّر قيمةً محاسبية وقعت.
--
...
--    shift_id, receipt_number, patient_id, idempotency_key, reversal_of_id).
--    لا يوجد في البرنامج اليوم أي UPDATE على payments — فالحارس لا يكسر
--    مسارًا قائمًا، ويمنع المستقبل وكل عميل خارجي. note/created_by metadata
--    غير محاسبية تبقى قابلة للتحديث.
--
--  * expenses: amount_minor, currency, exchange_rate, base_amount_minor,
--    base_currency, category, party_id, voucher_number, shift_id محصّنة.
--    ⚠️ payable_id مستثنى عمدًا: مساران قائمان (تنظيف التزامات المختبر عند
--    الإلغاء، lib/db.ts:3458 و4953) يضبطانه NULL — رابطُ سير عملٍ لا قيمة
--    محاسبية.
--
--  * inventory_movements: item_id, kind, qty, unit_cost_minor, is_return,
--    visit_id, patient_id, expiry_date محصّنة — فمتوسط التكلفة الموزون يُشتق
--    من الحركات كاملةً، وتعديل حركة تاريخية يُفسد كل WAC اللاحق بصمت.
--
...
--    ON CONFLICT DO UPDATE صراحةً لتجنب تضاعف الدَّين عند الإدخال المزدوج؛
--    التصحيح ليس صامتًا (updated_at + created_by يتغيّران معه).
--
-- DELETE ليس محصّنًا هنا: مساراته القائمة محكومة في التطبيق (deleteExpense
-- للأدمن بقيود وردية؛ حذف مريض شامل)، وaudit_log نفسه محصّن من التعديل
-- والحذف بtriggers قائمة من قبل (db.ts baseline).
