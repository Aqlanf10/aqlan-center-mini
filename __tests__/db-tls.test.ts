...
describe("قرار TLS لاتصال PostgreSQL", () => {
  it("sslmode=disable ⇒ بلا تشفير مع مضيف بعيد خارج سياق الإنتاج، مع تحذير", () => {
  ...
  it("sslmode=disable على مضيف بعيد في سياق إنتاج ⇒ رفض فوري بنيوي", () => {
  ...
  it("productionRuntime الصريح يرفض disable البعيد، وتعطيله الصريح يسمح", () => {
  ...
  it("بعيد بلا CA ⇒ تشفير بلا تحقق + تحذير", () => {
  ...
  it("CA من ملف قابل للقراءة ⇒ تحقق كامل", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "aqlan-tls-"));
    try {
      const caPath = path.join(dir, "ca.pem");
      await writeFile(caPath, TEST_CA);
  ...
  it("rootCertPem الصريح يتقدم على متغير/مسار الملف", () => {
  ...
  it("PEM مباشر غير صالح ⇒ رفض فوري ولا سقوط إلى unverified", () => {
  ...
  it("CA مضبوط لكن غير قابل للقراءة ⇒ خطأ فوري", () => {
    expect(() => decideTls(REMOTE, { rootCertPath: "/nonexistent/ca.pem" })).toThrow(/PGSSL_ROOT_CERT/);
    expect(() => decideTls(REMOTE, { rootCertPath: "/nonexistent/ca.pem" })).toThrow(/مرفوض/);
  });

  it("PGSSL_ROOT_CERT من البيئة يُحترم", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "aqlan-tls-"));
    try {
      const caPath = path.join(dir, "ca.pem");
      await writeFile(caPath, TEST_CA);
      vi.stubEnv("PGSSL_ROOT_CERT", caPath);
      expect(decideTls(REMOTE).mode).toBe("verified");
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });

  it("sslmode=verify-full بلا CA ⇒ رفض فوري", () => {
  ...
  it("sslmode=require بعيد بلا CA ⇒ تشفير بلا تحقق", () => {
  ...
  it("يفكّ هوية الهدف بلا كلمة سر", () => {
    expect(parseDatabaseHost(REMOTE)).toEqual({ host: "db.example.com", port: "5432", database: "aqlan_prod", user: "u" });
